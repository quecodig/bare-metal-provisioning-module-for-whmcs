...
function Hivelocity_ClientAreaCustomButtonArray($params) {
	$_LANG = \Hivelocity\classes\Helpers::lang('hivelocity');
	...
	try {
		...
		if ($devicePowerStatus == 'ON') {
			unset($deviceActions[$_LANG["boot"]]);
		} else if ($devicePowerStatus == 'OFF') {
			unset($deviceActions[$_LANG["reboot"]]);
			unset($deviceActions[$_LANG["shutdown"]]);
		}
