...
class Helpers {
    ...
    static public function maintenanceDatabase() {
        ...
        $query =  " CREATE TABLE IF NOT EXISTS HivelocityProductPrices("
				...
				. "     disabled_billing_periods TEXT DEFAULT NULL "
                . " );";
    ...
    static public function saveHivelocityProductPrice($hivelocityProductId, $hivelocityProductPrice, $disabled_billing_periods) {
        
        $pdo = Capsule::connection()->getPdo();
        $pdo->beginTransaction();
        $query =  "INSERT INTO HivelocityProductPrices (hivelocityProductId, hivelocityProductPrice, disabled_billing_periods) VALUES (?,?,?) ON DUPLICATE KEY UPDATE hivelocityProductPrice = ?;";
        $statement = $pdo->prepare($query);
        $statement->execute([$hivelocityProductId, $hivelocityProductPrice, $disabled_billing_periods, $hivelocityProductPrice]);
        $pdo->commit();
    }
    ...
    static public function lang( $phrase) {
        $lenguageDir = dirname(__FILE__). DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . "lang" . DIRECTORY_SEPARATOR;
        $config = isset($GLOBALS["CONFIG"]) ? $GLOBALS["CONFIG"] : [];
        $lenguage = isset($_SESSION["Language"]) ? $_SESSION["Language"] : (isset($config["Language"]) ? $config["Language"] : "english");
        $lenguageFile = file_exists($lenguageDir . $lenguage . ".php") ? $lenguage : "english";

        if(file_exists($lenguageDir . 'english.php')){
            include_once($lenguageDir . 'english.php');
        }

        require $lenguageDir . $lenguageFile . ".php";
        return isset($_LANG) ? $_LANG[$phrase] : [];
    }
}
