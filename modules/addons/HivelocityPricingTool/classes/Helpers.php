...
class Helpers {
    /**
     * Actualizar precios de productos en todos los ciclos de facturación.
     *
     * @param int $productId ID del producto
     * @param float $monthlyPrice Precio mensual base
    ...
    static public function updateAllProductPricing($productId, $pricingData, $currencyId, $disabledBillingPeriods = []) {
        ...
        // Verificar si ya existe el registro de precios
        ...
        if ($row) {
            ...
            // Actualizar solo los ciclos habilitados
            $updateFields = [];
            $updateValues = [];
            ...
            if (!empty($updateFields)) {
                $updateValues[] = $priceId;
                $pdo->beginTransaction();
                $query = "UPDATE tblpricing SET " . implode(", ", $updateFields) . " WHERE id = ?";
                $statement = $pdo->prepare($query);
            ...
            // Insertar nuevos precios para los ciclos habilitados
            $insertFields = ["type", "currency", "relid"];
            $insertValues = ["product", $currencyId, $productId];
            $placeholders = ["?", "?", "?"];
            ...
            if (count($insertFields) > 3) {
                $pdo->beginTransaction();
                $query = "INSERT INTO tblpricing (" . implode(", ", $insertFields) . ") VALUES (" . implode(", ", $placeholders) . ")";
                $statement = $pdo->prepare($query);
